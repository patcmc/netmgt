...
RSpec.describe "home/index.html.erb", type: :view do
  ...
  describe "Welcome Section" do
    ...
    it "includes the 'Get Started' button with correct classes and text" do
      expect(page).to have_css('button.bg-white.text-primary.hover\\:bg-secondary.px-8.py-3.rounded-full.text-lg.font-semibold.shadow.transition.duration-300.ease-in-out', text: 'Get Started')
    end
  end
    ...
    it "displays three feature cards with correct titles and descriptions" do
      expect(page).to have_css('div.grid.grid-cols-1.md\\:grid-cols-3.gap-8.text-center') do |grid|
        ...
        within(feature_cards[0]) do
          ...
          expect(page).to have_css('p.text-gray-600.mt-2', text: 'Monitor your network performance and get real-time updates on network health and traffic.')
        end

        # Feature 2
        within(feature_cards[1]) do
          expect(page).to have_css('h2.text-lg.font-semibold.text-gray-700', text: 'Device Management')
          expect(page).to have_css('p.text-gray-600.mt-2', text: 'Manage all your network devices from a single dashboard with advanced configuration options.')
        end

        # Feature 3
        within(feature_cards[2]) do
          expect(page).to have_css('h2.text-lg.font-semibold.text-gray-700', text: 'Security Alerts')
          expect(page).to have_css('p.text-gray-600.mt-2', text: 'Receive instant alerts for network vulnerabilities and threats, ensuring your network\'s security.')
        end
      end
    end
  end
    ...
    it "displays three testimonials with correct quotes and authors" do
      expect(page).to have_css('div.grid.grid-cols-1.md\\:grid-cols-3.gap-8.mt-6') do |grid|
        ...
        within(testimonials[0]) do
          expect(page).to have_css('p.text-gray-600.italic', text: '"NetMgt has simplified our network operations and increased our response times dramatically."')
          expect(page).to have_css('div.mt-4.text-gray-700.font-semibold', text: 'John Doe, Network Admin')
        end

        # Testimonial 2
        within(testimonials[1]) do
          expect(page).to have_css('p.text-gray-600.italic', text: '"Thanks to NetMgt, we now have a secure and reliable network system in place."')
          expect(page).to have_css('div.mt-4.text-gray-700.font-semibold', text: 'Jane Smith, IT Manager')
        end

        # Testimonial 3
        within(testimonials[2]) do
          expect(page).to have_css('p.text-gray-600.italic', text: '"The real-time monitoring tools are a game changer for our network health management."')
